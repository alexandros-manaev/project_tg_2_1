...
def filter_vegetable_and_fruit_products(products):
    ...
    for product, weight, *rest in products:
        logging.debug(f"[DEBUG] Обрабатываем продукт: {product} с массой {weight}")
    ...
    logging.debug(f"[DEBUG] Отфильтрованные растительные продукты: {vegetable_and_fruit_products}")
    return vegetable_and_fruit_products

def classify_vegetable_and_fruit_product(product_name):
    lower_name = product_name.lower()
    logging.debug(f"[DEBUG] Проверяем название продукта: {product_name}, в нижнем регистре: {lower_name}")
...
def handle_vegetables_and_fruits(products):
    ...
    for product, weight, product_class in plant_products:
        ...
        logging.debug(f"[DEBUG] Продукт: {product}, масса: {weight}")
    logging.debug(f"[DEBUG] Общая масса растительной продукции: {total_weight:.2f} г")
