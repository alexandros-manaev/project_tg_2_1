...
def add_or_update_daily_omega(user_id, omega_value):
    ...
    # Форматируем значение с точкой в качестве десятичного разделителя
    omega_str = f"{omega_value:.2f}"  # например, "0.80"
    ...
    for record in all_values[1:]:
        try:
            ...
            if record_date.date() >= start_of_week.date() and record[user_index] == uid_str:
                # Явно заменяем запятую на точку – важно именно заменить, а не удалить символ
                value_str = record[omega_index].replace(',', '.')
                total_omega += float(value_str)
